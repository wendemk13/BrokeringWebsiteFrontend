...
function FeaturedHouses() {
    ...
    const fetchHouses = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/house`
        );
  ...
  if (properties.length === 0)
    return (
      <div className="listof-empty">{t(`No featured houses available`)}</div>
    );
          ...
          <div className="listof-featured-card" key={property.id}>
            <div className="listof-featured-image-container">
              ...
              <div className="listof-featured-badges">
                ...
                <span
                  className={`listof-type-badge ${
                    property.ForSellRent
                      ? property.ForSellRent.toLowerCase()
                      : ""
                  }`}
                >
                  {property.ForSellRent === "Rent" ? (
                    <>{t("For Rent")}</>
                  ) : (
                    <>{t("For Sell")}</>
                  )}
