...
const SearchPage = () => {
  ...
  {!['car', 'House'].includes(filters.propertyType) && t('Find Your Perfect Property')}
</h1>
      ...
      <div className="search-content">
        ...
        <div className={`filters-sidebar ${showMobileFilters ? 'mobile-visible' : ''}`}>
          ...
          <div className="filter-section">
            <h4><FontAwesomeIcon icon={faMoneyBillWave} /> {t(`Listing Type`)}</h4>
          ...
          <div className="filter-section">
            <h4><FontAwesomeIcon icon={filters.propertyType === 'car' ? faCar : faHome} /> {t(`Property Type`)}</h4>
            <div className="filter-options">
          ...
          <div className="filter-section">
            <h4><FontAwesomeIcon icon={faMapMarkerAlt} /> {t(`Location`)}</h4>
            <input
              type="text"
              name="location"
    ...
    <option value="Daewoo">Daewoo</option>
        ...
        <div className="results-section">
                <div className="results-header">
                  <h2>
                    {pagination.total} {pagination.total === 1 ? <>{t('Result Found')}</> : <>{t('Results Found')}</>} 
                    {filters.propertyType !== 'all' && (
                      <span>  {filters.propertyType === 'car' ? <>{t('for Cars')}</> : <>{t('for Houses')}</>}</span>
                    )}
                    {filters.listingType !== 'all' && (
                      <span>  {filters.listingType === 'rent' ? <>{t('to Rent')}</> : <>{t('to Buy')}</>}</span>
                    )}
                  </h2>
                </div>
                    ...
                    <div className="results-grid">
                      {searchResults.map(item => (
                        <div key={item.id} className="property-card" onClick={() => handleViewDetails(item.id, item.propertyType)}>
                           <div>
                            {item.ForSellRent === 'Rent' ? <span  className="card-badge-blue">{t(`For Rent`)}</span> : <span  className="card-badge-red">{t(`For Sale`)}</span>}
                          </div> 
